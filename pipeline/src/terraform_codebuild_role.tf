...
resource "aws_iam_role" "terraform_codebuild_role" {
  name = "${var.role}-${var.component}-${var.estate_id}-TestApply_Terraform_Role"
...
resource "aws_iam_policy" "terraform_codebuild_policy" {
  name        = "${var.role}-${var.component}-${var.estate_id}-TestApply_Terraform_Policy"
  path        = "/service-role/"
  description = "Policies needed by the CodeBuild project for TestApply the ${var.role}-${var.component}-${var.estate_id} project"
...
resource "aws_iam_policy_attachment" "terraform_codebuild_attachment" {
  name       = "${var.role}-${var.component}-${var.estate_id}-TestApply_Terraform_Attachment"
